...
const GalleryPage = ({ username }) => {
  ...
  const [tags, setTags] = useState('sri lanka, kandy, nuwara eliya, travels')
  ...
  const handleCreate = () => {
    try {
      ...
      createGallery({
        data: {
          title, longTitle, baseUrl, description, coverImg, tags: tagsArray, ratios: ratios.split(',').map(ratio => Number(ratio))
        },
        callback: (message) => {
          setMessage(message)
        }
